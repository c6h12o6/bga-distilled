...
class Recipe {
    ...
    function __construct($name, $tier, $sp, $region, $isCraftable) {
        $this->name = $name;
        $this->sp = $sp;
        $this->tier = $tier;
        $this->region = $region;
...
Purchase it or return it to the bottom of the deck.`
        ...
        $this->signature_ing_cards = array(
            new Card(1, 'Sangiovese Grapes', 0, 2, 4, CardType::SUGAR, Sugar::FRUIT ),
            new Card(2, 'Dried Juniper Berries', 0, 2, 4, CardType::SUGAR, Sugar::FRUIT ),
            new Card(3, 'Blue Agave', 0, 2, 4, CardType::SUGAR, Sugar::PLANT ),
            new Card(4, 'Quebranta Grapes', 0, 2, 4, CardType::SUGAR, Sugar::FRUIT ),
            new Card(5, 'Almond Potatoes', 0, 2, 4, CardType::SUGAR, Sugar::PLANT ),
            new Card(6, 'Sugarcane Juice', 0, 2, 4, CardType::SUGAR, Sugar::PLANT ),
            new Card(7, 'Common Juniper Berries', 0, 2, 4, CardType::SUGAR, Sugar::FRUIT ),
            new Card(8, 'Nipa Palm', 0, 2, 4, CardType::SUGAR, Sugar::PLANT ),
            new Card(9, 'Sugarcane Molasses', 0, 2, 4, CardType::SUGAR, Sugar::PLANT ),
            new Card(10, 'Sorghum Bicolor', 0, 2, 4, CardType::SUGAR, Sugar::GRAIN ),
            new Card(11, 'Dent Corn', 0, 2, 4, CardType::SUGAR, Sugar::GRAIN ),
            new Card(12, 'Winter Rye', 0, 2, 4, CardType::SUGAR, Sugar::GRAIN ),
            new Card(13, 'Sinica Rice', 0, 2, 4, CardType::SUGAR, Sugar::GRAIN ),
            new Card(14, 'Unmalted Barley', 0, 2, 4, CardType::SUGAR, Sugar::GRAIN ),
            new Card(15, 'Bere Barley', 0, 2, 4, CardType::SUGAR, Sugar::GRAIN ),
            new Card(16, 'Pearl Millet', 0, 2, 4, CardType::SUGAR, Sugar::GRAIN ),
            new Card(17, 'Buck Wheat', 0, 2, 4, CardType::SUGAR, Sugar::GRAIN ),
            new Card(18, 'Ugni Blanc Grapes', 0, 2, 4, CardType::SUGAR, Sugar::FRUIT ),
        );
        ...
        $this->signature_recipes = array(
            array('name' => 'Premium Dark Rum',
                    ...
                    'signature' => true,
                    'allowed' => array(Sugar::PLANT)),
            array('name' => 'Straight Bourbon Whiskey',
                    'sp' => 18,
                    'sugars' => 3,
                    'value' => 1,
                    'aged' => true,
                    ...
                    'signature' => true,
                    'allowed' => array(Sugar::GRAIN)),
            array('name' => 'Oceania-Strength Gin',
                    'sp' => 10,
                    'sugars' => 3,
                    'value' => 1,
                    'aged' => false,
            ...
            array('name' => 'Single Malt Whisky',
                    ...
                    'signature' => true,
                    'allowed' => array(Sugar::GRAIN)),
            array('name' => 'Extra Anejo Tequila',
                    'sp' => 19,
                    'sugars' => 3,
                    'value' => 1,
                    'aged' => true,
            ...
            array('name' => 'Single Pot Whiskey',
                    ...
                    'signature' => true,
                    'allowed' => array(Sugar::GRAIN)),
            array('name' => 'Canadian Rye Whisky',
                    'sp' => 16,
                    'sugars' => 3,
                    'value' => 1,
                    'aged' => true,
                    ...
                    'signature' => true,
                    'allowed' => array(Sugar::GRAIN)),
            array('name' => 'Peruvian Pisco Puro',
                    'sp' => 14,
                    'sugars' => 3,
                    'value' => 1,
                    'aged' => false,
                    ...
                    'signature' => true,
                    'allowed' => array(Sugar::FRUIT)),
            array('name' => 'Royal Officer\'s Whisky',
                    'sp' => 11,
                    'sugars' => 2,
                    'value' => 1,
                    'aged' => true,
            ...
            array('name' => 'Grappa Invecchiata',
                    ...
                    'signature' => true,
                    'allowed' => array(Sugar::FRUIT)),
            array('name' => 'Strong Aroma Báijiŭ',
                    'sp' => 15,
                    'sugars' => 3,
                    'value' => 2,
                    'aged' => true,
        ...
        $this->labelBenefits = array(
            "5 <span class='icon-coin-em'></span>",
            "Move signature ingredient to pantry",
            "Discard 1 card from pantry. Gain 1 card from truck",
            "1 free ingredient",
            "1 free recipe",
            "1 free item",
            "1 free distillery upgrade",
        );
    ...
    {    
        ...
        for ($i = 0; count($saList) < count($players) + 1; $i++) {
            ...
            if ($flight == 7 && ($sa->uid == 8 || $sa->uid == 21)) 
                continue;
            // On flight D with two players, do not  use quantity over quality
            if ($flight == 4 && count($players) == 2 && $sa->uid == 22)
                continue;
...
////////////    
    ...
    */
    ...
    function playerHasPowers($pid, $powers) {
        ...
        foreach ($powers as $checkPower) {
            ...
            foreach ($powerCards as $pc) {
                if ($pc['uid'] == $checkPower && $pc['player_id'] == $pid)  {
                    $found = true;
                    break;
                }
            }
    ...
    public function canPlayerBuy($playerId) {
        ...
        if ($basics> 0) {
            /*
            self::notifyAllPlayers("dbgdbg", '${player_name} could in theory buy a basic card', array(
        ...
        //self::notifyAllPlayers("dbgdbg", 'cards', array("cards" => $cards));
    ...
    public function notifyAllPlayersOfPowerCardGivingBasicCard($pid, $card_id, $pc, $location = null) {
        ...
        $this->notifyAllPlayers("buyCard", _('${player_name} gets ${card_name} from ${trigger_name}'), array(
    ...
    public function checkSpiritAwards() {
        ...
        foreach ($saList as $sa) {
            ...
            case 2: // Land Grab
                ...
                foreach ($playerLabels as $player_id => $recipes) {
                    $result = array();
                    foreach ($recipes as $r) {
                        $region = $this->getRecipeRegionForPlayer($player_id, $r);
                        self::notifyAllPlayers("dbgdbg", 'DEBUG LANDGRAB: ${name} is from ${region}', array(
                            'name' => $r['name'],
                            'region' => $region,
                        ));
                    ...
                    self::notifyAllPlayers("dbgdbg", 'DEBUG LANDGRAB: Region count is ${count}', array(
                        'count' => count($result),
                    ));
                    if (count($result) >= 2) {
                        $winningPlayers[$player_id] = true;
                    }
                }
            ...
            case 4:
                ...
                foreach ($players as $p => $info) {
                    $labels = self::dbQuery("SELECT * FROM label WHERE location='player' AND player_id=$p");
                    $regions = array();
                    foreach ($labels as $l) {
    ...
    function dealDistillers($player_id) {
        ...
        for ($i = 0; $i < 2; $i++) {
            ...
            self::notifyPlayer($player_id, "revealDistiller", _('${player_name} may select ${distiller_name}'), array(
                'distiller_name' => $distiller->name,
                'player_id' => $player_id,
                'player_name' => $this->getPlayerName($player_id),
    ...
    function dealStartingCards($player_id) {
        ...
        for( $i = 19; $i <= 20; $i++) {
            ...
            /*$sql = sprintf("INSERT INTO premium_item (uid, location, location_idx, player_id) 
                            VALUES (%d, 'player', 1, '%d')", $newC->uid, $player_id);
            self::DbQuery($sql);*/
        }
        // Goals
        for ($i = 0; $i < 3; $i++) {
    ...
    function checkPlayerCanBuy($playerId, $cost, $marketId) {
        ...
        if ($result['money'] < $cost) {
            ...
            self::notifyAllPlayers( "moneyMoney", _('${playerId} does not have ${money} money'), array(
                'playerId' => $playerId,
                'money' => $cost
            ));
        }
    ...
    function playerGains($playerId, $amount, $trigger_name, $trigger_uid=null) {
        ...
        if ($trigger_name) {
            ...
            if ($amount >= 0) {
                self::notifyAllPlayers( "playerGains", _('${player_name} gains ${amount} <span class="icon-coin-em"></span> from ${card_name}'), array(
                    'amount' => $amount,
                    'player_name' => $this->getPlayerName($playerId),
                    'player_id' => $playerId,
                ...
                self::notifyAllPlayers( "playerGains", _('${player_name} loses ${normal_amount} <span class="icon-coin-em"></span> from ${card_name}'), array(
                    'amount' => $amount,
                    'player_name' => $this->getPlayerName($playerId),
                    'player_id' => $playerId,
                    'card_name' => $trigger_name,
    ...
    function playerPoints_internal($playerId, $amount, $notif, $trigger_name=null) {
        ...
        if ($trigger_name) {
            $normalAmount = abs($amount);
            if ($amount >= 0) {
                self::notifyAllPlayers( $notif, _('${player_name} gains ${sp} <span class="icon-sp-em"></span> from ${trigger_name}'), array(
                    'sp' => $amount,
                    'player_name' => $this->getPlayerName($playerId),
                    'player_id' => $playerId,
                ...
                self::notifyAllPlayers( $notif, _('${player_name} loses ${normal_amount} <span class="icon-sp-em"></span> from ${trigger_name}'), array(
                    'sp' => $amount,
                    'player_name' => $this->getPlayerName($playerId),
                    'player_id' => $playerId,
                    'trigger_name' => $trigger_name,
    ...
    function playerPointsEndgame($playerId, $amount, $row, $trigger_name) {
        $sql = sprintf("UPDATE player SET player_score=player_score+%d WHERE player_id='%s'",
                        $amount, $playerId);
        self::DbQuery($sql);
        ...
        if ($trigger_name) {
            self::notifyAllPlayers( 'playerPointsEndgame', _('${player_name} gains ${sp} SP from ${trigger_name}'), array(
                'sp' => $amount,
                'player_name' => $this->getPlayerName($playerId),
                'player_id' => $playerId,
                'row' => $row,
...
//////////// 
    ...
    */
    ...
    function buyCard2($uid, $market, $slotId=null, $duSlot=null) {
        $this->buyCard_internal($this->getActivePlayerId(), $uid, $market, $slotId, $duSlot);
    }
    function buyCard_internal($playerId, $uid, $marketId, $slotId=null, $duSlot=null, $powers=null) {
        $free = false;
        if ($this->getStateName() == "placeLabel3" || 
            $this->getStateName() == "placeLabel4" || 
            $this->getStateName() == "placeLabel5" || 
            $this->getStateName() == "placeLabel6" || 
        ...
        if ($marketId != 'bm' && !$this->isCardInMarket($deck, $uid)) {
        ...
        if ($marketId == 'bm') {
            ...
            $this->notifyAllPlayers("buyCard", _('${player_name} gets ${card_name} from the basic market'), array(
                'player_name' => $this->getPlayerName($playerId),
                'player_id' => $playerId,
                'uid' => $newUid,
                'card_name' => $card_name,
            ...
            if ($slotId != 0) {
                ...
                self::notifyAllPlayers("updateMarket", _('All ${market} cards shift to the right. ${card_name} revealed'), array(
                    'market' => $market,
                    'market_id' => $marketId,
    ...
    function waterReveal($deckName) {
        ...
        for ($ii = 0; $ii < $count; $ii++) {
            ...
            self::notifyAllPlayers('revealCards', _('${player_name} reveals ${card_name} from ${market_name}'), array(
                'player_id' => $pid,
    ...
    function stPlayerBuyTurnRevealCleanup($nextState = true) {
        ...
        if ($affected) 
            self::notifyAllPlayers("endReveal", "Revealed cards are returned to the bottom of the deck", array());
        else 
            self::notifyAllPlayers("endReveal", "End of reveal", array());
    ...
    function distillAgain($triggerCard) {
        ...
        foreach ($relevantCards as $rc) {
            /*
            if ($rc['uid'] == 9999) { // trade in card
                continue;
            }
            */
            $wbCards[] = $rc['uid'];
            if ($this->AllCards[$rc['uid']]->type == CardType::SUGAR) 
               $sugar++;
        }
        ...
        "));
        self::dbQuery("UPDATE bottomless_card SET used=1,location='trash' WHERE card_id=0 AND  turn=$turn");
        ...
        self::notifyAllPlayers("moveToWashback", "Removing alcohols before distilling again", array(
            'cards' => $wbCards,
            'player_id' => $pid,
        ));
    ...
    function skipDistill($activePlayer = null) {
        ...
        self::notifyAllPlayers("skipDistill", _('${player_name} skips distill phase'), array(
            'player_id' => self::getActivePlayerId(),
            'player_name' => self::getActivePlayerName()
        ));
        $turn = self::getGameStateValue("turn");
    ...
    function skipSale() {
        ...
        self::notifyAllPlayers("skipSale", _('${player_name} passes'), array(
            'player_id' => self::getActivePlayerId(),
            'player_name' => self::getActivePlayerName(),
        ));
    ...
    function trade($in, $out) {
        ...
        if ($this->AllCards[$out]->card_id < 133 || $this->AllCards[$out]->card_id > 138)  // Cannot trade yeast
    ...
    function placeLabel2Trade($in, $out, $truck, $duSlot) {
        ...
        if (1 != self::getUniqueValueFromDb("SELECT COUNT(*) FROM (
            ...
            UNION ALL SELECT uid, player_id, location FROM bottomless_card
                ...
                throw new BgaSystemException("Trade card not valid");
        }
        if (1 != self::getUniqueValueFromDb("SELECT COUNT(*) FROM (
            SELECT uid, player_id, location FROM premium_ingredient
            UNION ALL SELECT uid, player_id, location FROM bottomless_card
        ...
        if ($truck == 'du') {
            ...
            self::notifyAllPlayers("placeLabel2Trade", 
                _('${player_name} discards ${out_name} and draws ${in_name} from the ${truck_name} truck'), array(
                'player_name' => self::getActivePlayerName(),
                'player_id' => self::getActivePlayerId(),
                'in_name' => $cardIn->name,
    ...
    function selectRoundStartAction($uid) {
        ...
        if ($uid == 0) {
            $card_id = $this->getPlayerDistiller($pid);
        } else {
            $card_id = $this->AllCards[$uid]->card_id;
        }
        if (!$this->playerHasPowers($pid, [$card_id])) {
            ...
            switch ($pc['card_id']) {
                case 120: // warehouse manager
                    ...
                    self::notifyAllPlayers("powerCard", 
                        _('${player_name} gets an additional ${value} from ${trigger_name}'), 
                        array(
                            'player_name' => $this->getPlayerName($playerId),
                            'player_id' => $playerId,
                            'value' => $maxFlavor,
    ...
    function sellDrink_internal($drinkId, $bottle, $labelSlot, $optForSp, $sellArgs) {
        ...
        if (self::getUniqueValueFromDb("SELECT COUNT(*) FROM label WHERE location='player' AND location_idx=$labelSlot"))
        ...
        self::notifyAllPlayers("chooseBottle", _('${player_name} uses ${card_name}'), array(
            'player_name' => self::getActivePlayerName(),
            'player_id' => self::getActivePlayerId(),
        ...
        if ($this->AllCards[$bottle]->card_id == 20) {
            $this->notifyAllPlayers("sellDrink", _('${player_name} sold ${recipe_name} for ${value} <span class="icon-coin-em"></span> and ${sp} <span class="icon-sp-em"></span>. Money Breakdown:
        ...
        if ($info["label_count"] > 0 || $info['recipe_slot'] == 7) {
            ...
            if ($optForSp || true) {
                self::notifyAllPlayers("placeLabel", _('${player_name} places label'), array(
                    'player_name' => self::getActivePlayerName(),
                    'player_id' => self::getActivePlayerId(),
                    'slot' => $labelSlot,
                    'location' => $info["location"],
            ...
            self::notifyAllPlayers("placeLabel", _('No label to place for ${player_name}'), array(
                'player_name' => self::getActivePlayerName(),
                'player_id' => self::getActivePlayerId(),
                'hasLabel' => false,
                'location' => $info["location"],
    ...
    function selectRecipe($slot, $drinkId, $barrelUid) {
        ...
        //$slot = $this->getRecipeSlotFromName($recipeName, $pid);
        ...
        self::notifyAllPlayers("selectRecipe", _('${player_name} distilled ${recipe_name} in ${card_name}'), array(
            "player_name" => self::getActivePlayerName(),
            "player_id" => $pid,
            "recipe_name" => $recipeName,
            "recipe" => $r,
    ...
    */
    ...
    function argDistillPowers() {
        ...
        foreach ($powerCards as $pc) {
            ...
            switch ($tmp->card_id) {
                case 104:
                    $tmp->description = "Add 2 additional alcohol";
                    break;
                case 116:
                    $tmp->description = "Remove only top card";
                    break;
                case 129:
                    $tmp->description = "Add 1 additional alcohol";
                    break;
            }
            $result[] = $tmp;
        }
    ...
    function argChooseBottle() {
        ...
        return array("bottles" => $bottles, "drinkId" => $drinks[0].id);
    }
    ...
    */
    function stEndGameScoring() {
        ...
        foreach ($players as $player_id => $info) {
            ...
            foreach ($spirits as $spirit) {
                ...
                $this->playerPointsEndgame($player_id, $value['total'], "warehouses", $spirit['location']);
        ...
        foreach ($players as $player_id => $info) {
            ...
            foreach ($regions as $r => $count) {
                ...
                if ($score > 0)
                    $this->playerPointsEndgame($player_id, $score, "bottles", "$r bottles");
            }
            if (count($regions) >= 3)
                $this->playerPointsEndgame($player_id, 5, "bottles", "bottles from mulitple regions");
        }
            ...
            foreach ($dus as $id => $du) {
                ...
                switch ($du['card_id']) {
                    ...
                    case 129: // Coppersmith
                        ...
                        break;
                    case 131: // Cooper
                        $labels = self::getCollectionFromDb("SELECT COUNT(*) FROM label WHERE player_id=$player_id AND (location='player' OR location LIKE 'warehouse%')");
                        $barrels = array();
                        foreach ($labels as $luid => $labelInfo) {
                            $barrels[$this->AllCards[$luid]->subtype] = true;
                        }
                        $score = count($barrels);
                        break;
                    case 132: // Biochemist
                ...
                if ($printResult) {
                    $this->playerPointsEndgame($player_id, $score, "dus", $this->AllCards[$uid]->name);
        ...
        foreach ($players as $player_id => $info) {
                ...
                switch ($goalInfo['card_id']) {
                    ...
                    case 156: // East Champion
                        ...
                        foreach ($players as $pid => $info) {
                            ...
                            foreach ($lbls as $recipe ) {
                                $region = $this->getRecipeRegionForPlayer($pid, $recipe);
                                if ($region == $match)
                                    $playerCount++;
                            }
                    ...
                    case 153: // Close to Home
                        ...
                        foreach ($players as $pid => $info) {
                            $playerScores = array();
                            foreach ($playerLabels[$pid] as $label) {
                                if (!array_key_exists($label['region'], $playerScores)) {
                                    $playerScores[$label['region']] = 0;
                                }
                                $playerScores[$label['region']]++;
                            }
                            foreach ($playerScores as $color => $ct) {
                                if (!array_key_exists($color, $maxScores)) {
                                    $maxScores[$color] = $ct;
            ...
            $this->playerPointsEndgame($player_id, $score, "money", "converting money to SP");
    ...
    function stDistillFinalize() {
        ...
        foreach ($playerList as $pid => $l) {
            $sql = sprintf('INSERT INTO drink(cards, player_id, location)
                            VALUES ("%s", "%d", "washback")', 
                            implode(",", $l),
                            $pid);
            self::DbQuery($sql);
                ...
                if ($pc['market'] == 'du') {
                    switch ($pc["card_id"]) {
                        ...
                        case 113: // Doig Ventilator
                            ...
                            self::notifyPlayer($pid, "doig", _('${trigger_name} reveals ${card1_name} and ${card2_name}. ${winner_name} selected.'), array(
                                "trigger_name" => $this->AllCards[$pc['uid']]->name,
    ...
    function stMarketEnd() {
        ...
        self::notifyAllPlayers("updateMarkets", 
            _('Markets update at the end of the market phase, ${new_cards} added'), 
            array(
    ...
    function stPlaceLabelForSp() {
        ...
        self::notifyAllPlayers("playerPoints", _('${player_name} gains ${sp} points from label rewards'), array(
            'player_name' => self::getActivePlayerName(),
            'player_id' => self::getActivePlayerId(),
            'sp' => 2,
        ));
    ...
    function stPlaceLabel1() {
        ...
        self::notifyAllPlayers("moveSignature", 
            _('${player_name} moves signature ingredient ${card_name} to the pantry'),
            array(
                'player_name' => self::getActivePlayerName(),
                'player_id' => self::getActivePlayerId(),
                'card_name' => $sigCard->name,
        ...
        foreach ($powerCards as $pc) {
            ...
            switch ($pc['card_id']) {
                case 10: // Angus
                    ...
                    self::notifyPlayer($playerId, 'revealCards', _('${player_name} reveals ${card_name} from flavor deck'), array(
                        'player_id' => $playerId,
                        'player_name' => self::getActivePlayerName(),
                        'card_id' => $c1,
...
//
    ...
    /* leaky absctractions from Deck because I dont have access to a bunch of functions I need */
    function reshuffle($deck) {
        ...
        self::notifyAllPlayers("shuffleDeck", _('Shuffling ${deck_name} truck into deck'), array(
            'deck_name' => $deck->name,
    ...
    function revealFromDeck($deck) {
        ...
        $this->reshuffle($deck);
        return $this->revealFromDeck($deck, $index);
    }
    ...
    function getPlayerCardList($player_id, $isCurrent) {
        ...
        $sql = sprintf("SELECT flavor_count, location, id FROM drink
    ...
    function getDistillableRecipes($uids) {
        $pid = self::getActivePlayerId();
        ...
        foreach($allRecipes as $r) {
            ...
            // TODO check to see if this player has already made their signature drink
            if ($include != null && !$includeFound)  { // signature ingredient not found
                //self::notifyAllPlayers("dbgdbg", "include", array("include" => $include));
                $reason[] = 'include';
                continue;
            }

            /* TODO allow distilling aged drinks with a full warehouse
            if ($r['aged']) {
                // get warehouse capacity
                $ct = self::getUniqueValueFromDb("SELECT COUNT(*) as ct FROM drink WHERE location LIKE 'warehouse%' AND player_id=$pid AND sold=0");
                if ($ct >= 2) {
                    $reason[] = 'no space';
                    continue;
                }
    ...
    function distillRemoval($cards, $skipAddAlcohol, $activePowers) {
        ...
        if ($activePowers && in_array(116, $card_ids)) {
            ...
            self::notifyAllPlayers("removeCards", _('${card1_name} was removed from ${player_name}\'s washback.'), 
                array(
            ...
            self::notifyAllPlayers("removeCards", _('${card1_name} and ${card2_name} were removed from ${player_name}\'s washback.'), 
                array(
                    "card1_name" => $this->AllCards[$card1]->name,
    ...
    function addAlcohols($cards, $activePowers) {
        ...
        if ($activePowers) {
            ...
            foreach ($activePowers as $pc) {
                ...
                switch ($this->AllCards[$pc]->card_id) {
                    case 104: 
                        // Worm Tub
                        self::notifyAllPlayers("powerCard", _('Adding 2 additional alcohol (${trigger_name})'),
                            array('trigger_name' => $this->AllCards[$pc]->name));
                        $count += 2;
                        break;
                    case 129:
                        // Coppersmith
                        self::notifyAllPlayers("powerCard", _('Adding 1 additional alcohol (${trigger_name})'),
                            array('trigger_name' => $this->AllCards[$pc]->name));
                        $count += 1;
                        break;
                }
        ...
        self::notifyAllPlayers("addAlcohols", _('${player_name} adds ${count} ${card_name} to their spirit.'),
            array(
                "player_name" => $this->getActivePlayerName(),
                "player_id" => $this->getActivePlayerId(),
                "card_name" => "alcohol",
